...
import { Package, ChevronRight, Calendar, MapPin, Box } from 'lucide-react';

export default function ConsolidationOrders({ orders }) {
    const { language } = usePage().props;
    const __ = (key) => language?.[key] || key;

    return (
        <FanLayout>
            <Head title={`${__('Consolidation')} - CirclePort`} />
                ...
                <div className="mb-12 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-slate-500 font-bold uppercase tracking-widest text-xs mt-2">
                            {__('Items ready for consolidation and international shipping')}
                        </p>
                    </div>
                </div>
                ...
                {orders.length === 0 ? (
                    <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-[3rem] p-20 text-center">
                        ...
                        <p className="text-slate-400 font-black uppercase tracking-widest text-sm">
                            {__('No items waiting for consolidation')}
                        </p>
                    </div>
                ) : (
                    <div className="space-y-6">
                        {orders.map((order) => (
                            <Link
                                ...
                                className="group bg-white border border-slate-100 rounded-[2.5rem] overflow-hidden hover:shadow-xl hover:border-amber-500 transition-all"
                            >
                                <div className="p-8 flex flex-col lg:flex-row lg:items-center gap-6">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between">
                                            ...
                                            <div className="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center text-slate-300 group-hover:bg-amber-500 group-hover:text-white transition-all">
